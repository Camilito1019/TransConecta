...
<style>
	.page-shell { position: relative; padding: 22px 20px 60px 20px; color: var(--tc-text); }
	.bg-shape { position: absolute; border-radius: 999px; filter: blur(90px); opacity: 0.32; z-index: 0; }
	.shape-a { width: 420px; height: 420px; background: color-mix(in srgb, var(--tc-accent) 34%, transparent); top: -140px; left: -120px; }
	.shape-b { width: 360px; height: 360px; background: color-mix(in srgb, var(--tc-accent-2) 28%, transparent); bottom: -160px; right: -120px; }
	...
	.chip { padding: 8px 12px; border-radius: 999px; font-weight: 700; font-size: 13px; border: 1px solid color-mix(in srgb, var(--tc-accent) 26%, var(--tc-border)); background: color-mix(in srgb, var(--tc-accent) 10%, var(--tc-surface)); color: color-mix(in srgb, var(--tc-accent-2), var(--tc-text) 40%); }
	.chip.muted { background: var(--tc-surface); border-color: var(--tc-border-strong); color: var(--tc-text); }
	.chip.success { background: var(--tc-success-bg); border-color: var(--tc-success-border); color: var(--tc-success-text); }

	.hero-actions { display: flex; gap: 10px; }
	.primary { background: linear-gradient(135deg, var(--tc-accent), var(--tc-accent-2)); color: var(--tc-on-accent); border: 1px solid var(--tc-border-strong); border-radius: 12px; padding: 10px 14px; font-weight: 800; cursor: pointer; box-shadow: 0 12px 26px color-mix(in srgb, var(--tc-accent), transparent 70%); transition: transform 0.12s ease, box-shadow 0.18s ease; }
	.primary:hover { transform: translateY(-1px); box-shadow: 0 16px 30px color-mix(in srgb, var(--tc-accent), transparent 66%); }
	.primary:active { transform: translateY(0); box-shadow: 0 10px 22px color-mix(in srgb, var(--tc-accent), transparent 74%); }
	...
	.label { text-transform: uppercase; letter-spacing: 0.1em; font-size: 11px; color: color-mix(in srgb, var(--tc-text-muted), transparent 10%); margin: 0; font-weight: 800; }
	...
	.outline { color: var(--tc-text); border-color: var(--tc-border); }
	...
	.field input, .field select {
		padding: 12px 12px;
		border-radius: 12px;
		border: 1.5px solid color-mix(in srgb, var(--tc-border), transparent 10%);
	...
	.field input:focus, .field select:focus {
		...
		box-shadow: 0 10px 30px color-mix(in srgb, var(--tc-accent), transparent 86%);
		background: var(--tc-surface);
	}
	.doc-block { grid-column: 1 / -1; border: 1.5px dashed color-mix(in srgb, var(--tc-border), var(--tc-accent) 18%); background: color-mix(in srgb, var(--tc-accent) 7%, var(--tc-surface-2)); border-radius: 12px; padding: 12px; display: grid; gap: 10px; }
	.doc-head { display: flex; flex-wrap: wrap; align-items: center; gap: 8px; }
	.doc-hint { margin: 0; color: var(--tc-text-muted); font-size: 13px; }
	.doc-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12px; }
	.doc-field { background: var(--tc-surface); border: 1px solid var(--tc-border); border-radius: 12px; padding: 10px; display: grid; gap: 8px; }
	.doc-label { display: flex; justify-content: space-between; align-items: center; gap: 8px; font-weight: 700; color: var(--tc-text); }
	.doc-label-meta { display: flex; align-items: center; gap: 6px; }
	.doc-hint-light { color: color-mix(in srgb, var(--tc-text-muted), transparent 10%); font-weight: 600; font-size: 12px; }
	.doc-hint-light.strong { color: var(--tc-accent-2); }
	.doc-pill { display: inline-flex; align-items: center; gap: 6px; padding: 6px 10px; border-radius: 999px; background: var(--tc-success-bg); border: 1px solid var(--tc-success-border); color: var(--tc-success-text); font-size: 12px; font-weight: 700; }
	.doc-status { margin: 0; color: var(--tc-danger-text); font-weight: 700; font-size: 13px; }
	.doc-existing { display: flex; flex-wrap: wrap; gap: 8px; padding: 4px 2px 8px 2px; }
	.doc-existing.muted { color: color-mix(in srgb, var(--tc-text-muted), transparent 10%); font-size: 13px; }
	.doc-chip { display: inline-flex; align-items: center; gap: 8px; padding: 8px 12px; border-radius: 12px; border: 1px solid color-mix(in srgb, var(--tc-border), transparent 10%); background: var(--tc-surface); color: var(--tc-text); text-decoration: none; font-weight: 700; box-shadow: 0 6px 18px color-mix(in srgb, #000, transparent 92%); transition: transform 0.12s ease, box-shadow 0.18s ease; }
	.doc-chip.mini { padding: 4px 10px; font-size: 12px; background: color-mix(in srgb, var(--tc-accent) 12%, var(--tc-surface)); border-color: color-mix(in srgb, var(--tc-accent) 22%, var(--tc-border)); color: color-mix(in srgb, var(--tc-accent-2), var(--tc-text) 35%); }
	.doc-chip:hover { transform: translateY(-1px); box-shadow: 0 10px 26px color-mix(in srgb, #000, transparent 86%); }
	.doc-meta { color: color-mix(in srgb, var(--tc-text-muted), transparent 10%); font-size: 12px; font-weight: 600; }
	.loading-docs { color: var(--tc-accent-2); font-weight: 700; font-size: 13px; }
	...
	tbody td { padding: 14px; border-bottom: 1px solid color-mix(in srgb, var(--tc-border), transparent 45%); color: var(--tc-text); }
	tbody tr:hover { background: color-mix(in srgb, var(--tc-accent) 8%, var(--tc-surface-2)); }
	.actions { display: flex; gap: 6px; flex-wrap: wrap; justify-content: flex-end; }
	.status { padding: 6px 10px; border-radius: 10px; font-weight: 800; font-size: 12px; text-transform: capitalize; }
	.status.green { background: var(--tc-success-bg); color: var(--tc-success-text); border: 1px solid var(--tc-success-border); }
	.status.red { background: var(--tc-danger-bg); color: var(--tc-danger-text); border: 1px solid var(--tc-danger-border); }
	.status.blue { background: color-mix(in srgb, var(--tc-accent) 12%, var(--tc-surface)); color: color-mix(in srgb, var(--tc-accent-2), var(--tc-text) 35%); border: 1px solid color-mix(in srgb, var(--tc-accent) 22%, var(--tc-border)); }
	.status.amber { background: var(--tc-warning-bg); color: var(--tc-warning-text); border: 1px solid var(--tc-warning-border); }
	...
	.modal .lede.small { font-size: 13px; color: color-mix(in srgb, var(--tc-text-muted), transparent 5%); }
	...
	.card { background: linear-gradient(145deg, var(--tc-surface), color-mix(in srgb, var(--tc-surface-2), var(--tc-surface) 45%)); border: 1px solid color-mix(in srgb, var(--tc-border), transparent 10%); border-radius: 14px; padding: 12px; box-shadow: var(--tc-shadow); display: grid; gap: 8px; }
	...
	.doc-row .mini { font-size: 12px; padding: 6px 10px; border-radius: 10px; border: 1px solid color-mix(in srgb, var(--tc-accent) 20%, var(--tc-border)); background: color-mix(in srgb, var(--tc-accent) 10%, var(--tc-surface)); color: color-mix(in srgb, var(--tc-accent-2), var(--tc-text) 35%); text-decoration: none; font-weight: 700; }
	.doc-row .mini:hover { background: color-mix(in srgb, var(--tc-accent) 14%, var(--tc-surface)); }
	.pill { display: inline-flex; align-items: center; gap: 6px; padding: 6px 10px; border-radius: 999px; background: color-mix(in srgb, var(--tc-accent) 10%, var(--tc-surface)); color: color-mix(in srgb, var(--tc-accent-2), var(--tc-text) 35%); font-weight: 700; font-size: 12px; }
	.pill.muted-pill { background: var(--tc-surface-3); color: var(--tc-text-muted); }
	...
	@media (max-width: 720px) {
		...
		tbody tr { margin-bottom: 12px; border: 1px solid var(--tc-border); border-radius: 12px; padding: 10px; background: var(--tc-surface); }
