<script>
  ...
  async function cargarClientes() {
    clientes.update((c) => ({ ...c, loading: true }));
    try {
      const data = await clienteService.listar();
      clientes.update((c) => ({ ...c, items: data.clientes || [], loading: false }));
    } catch (error) {
      clientes.update((c) => ({ ...c, error: error.message, loading: false }));
  ...
  function resetForm() {
    editandoId = null;
    formData = { nombre: '', telefono: '', estado: 'activo' };
  }
  ...
  async function guardarCliente() {
    if (!formData.nombre) {
      addNotificacion('El nombre es obligatorio', 'warning');
      return;
    }
    ...
    try {
      if (editandoId) {
        ...
        addNotificacion('Cliente actualizado', 'success');
      } else {
        await clienteService.crear(payload);
        addNotificacion('Cliente creado', 'success');
      }
  ...
  function solicitarAccion(type, cliente) {
    confirmAction = {
      open: true,
      type,
  ...
  async function confirmarAccion() {
    ...
    try {
      if (confirmAction.type === 'eliminar') {
        await clienteService.eliminar(confirmAction.id);
        addNotificacion('Cliente eliminado', 'success');
      } else if (confirmAction.type === 'desactivar') {
        await clienteService.desactivar(confirmAction.id);
        addNotificacion('Cliente desactivado', 'success');
      } else if (confirmAction.type === 'activar') {
        await clienteService.activar(confirmAction.id);
        addNotificacion('Cliente activado', 'success');
      }
  ...
  $: stats = {
    ...
    activos: $clientes.items.filter((c) => c.estado === 'activo').length,
    inactivos: $clientes.items.filter((c) => c.estado === 'inactivo').length
  };
</script>

<svelte:head>
  <title>Gestión de Clientes - TransConecta</title>
</svelte:head>
...
<div class="page-shell">
  ...
  <section class="hero">
    <div class="hero-text">
      ...
      <h1>Clientes</h1>
      <p class="lede">Administra los clientes asociados a los trayectos.</p>
      <div class="chips">
    ...
    <div class="hero-actions">
      {#if puedeCrearClientes}
        <button class="primary" on:click={abrirFormulario}>+ Nuevo cliente</button>
      {/if}
    </div>
  </section>
  ...
  {#if mostrarFormulario}
    <section class="panel">
      ...
      <form class="form-grid" on:submit|preventDefault={guardarCliente}>
        <label class="field">
          <span>Nombre</span>
          <input type="text" placeholder="Cliente S.A." bind:value={formData.nombre} required />
        </label>
        <label class="field">
          <span>Teléfono</span>
  ...
  <section class="panel">
    ...
    {:else if $clientes.items.length === 0}
      <div class="empty">
              ...
              <tr>
                ...
                <td class="actions">
                  {#if puedeEditarClientes}
                    <button class="ghost" on:click={() => editarCliente(c)}>Editar</button>
                  {/if}
